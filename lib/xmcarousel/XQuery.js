...
(function(){
	...
	xQuery.fn = xQuery.prototype = {
		...
		init : function(selector){
			if (typeof selector === "string") {
				...
				for (var i = 0, len = nodeList.length; i < len; i++) {
					this[i] = nodeList[i];//类似数组对象
				}
				// 设置当前对象的 length
				this.length = len;//增加一个属性值
			...
			if (typeof attrName === "object") { // 将对象传递的数据作为CSS属性设置
				for (var prop in attrName) {
					for (var i = 0, len = this.length; i < len; i++) {
						this[i].style[prop] = attrName[prop];//给选出来的元素每一个设置属性值，这里是实现多个属性进行设置属性值
					}
				}
			} else {
				for (var i = 0, len = this.length; i < len; i++) {
					this[i].style[attrName] = attrValue;//这是单个属性进行属性值设置
				}
			}
			return this;
		},
		...
		animate : function(options, speed, fn){
			for (var i = 0, len = this.length; i < len; i++) {
				...
				for ( var attr in options ) { // 遍历options每个属性
					start[attr] = parseFloat(xQuery(element).css(attr));//这里是使用parseFloat是取消掉px
			...
			return this;//为了链式调用所以return this
		},
		//淡入
		 ...
		 * @param callback:回调函数
		 */
		on : function(type, callback){//当前元素已经包装
			for (var i = 0, len = this.length; i < len; i++) {
				var element = this[i];
		...
		innerHTML:function(val){
			if(this.length<=0){//当前xQuery中没有DOM对象
				return this;
			}
			if(!val){//获取第一个元素的值
				return this[0].innerHTML;
			}
			//设置每一个DOM元素的innerHTML
